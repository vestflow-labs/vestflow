...
export default function ScheduleView({ schedule, claimable: initialClaimable }: ScheduleViewProps) {
  const [claimable] = useState(initialClaimable);
  ...
  if (typeof window !== "undefined" && !shareUrl) {
    setShareUrl(`${window.location.origin}/schedule/${schedule.id}`);
  }

  const vestingProgress = () => {
    const now = Math.floor(Date.now() / 1000);
    if (now < schedule.start_time) return 0;
    if (schedule.duration <= 0) return 100;
  ...
  const handleSimulateClaim = async () => {
    ...
    try {
      const res = await fetch("/api/schedules/simulate", {
        ...
        body: JSON.stringify({ schedule_id: Number(schedule.id) }),
      });
      if (!res.ok) {
