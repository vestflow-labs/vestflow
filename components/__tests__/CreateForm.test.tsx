// @vitest-environment jsdom
import { describe, it, expect, vi, beforeEach } from "vitest";
import { render, screen, fireEvent, waitFor } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import CreateForm from "../CreateForm";

// ---------------------------------------------------------------------------
// Mocks
// ---------------------------------------------------------------------------

const mockCreateSchedule = vi.fn();

vi.mock("@/lib/stellar", () => ({
  createSchedule: (...args: any[]) => mockCreateSchedule(...args),
  parseContractError: (e: Error) => e.message,
  CONTRACT_ID: "CCZ6AE75C27DMB3SOIHK7WZSBUG3NQPVLHSVEBQ2FSAEVGRJ5TXAZWCX",
  NETWORK: "testnet",
  NATIVE_TOKEN: "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC",
}));

vi.mock("@/lib/WalletContext", () => ({
  useWallet: () => ({
    publicKey: "GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN",
  }),
}));

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/** Fill all required fields with valid values. */
async function fillValidForm(user: ReturnType<typeof userEvent.setup>) {
  await user.type(
    screen.getByPlaceholderText("G..."),
    "GBSOV3F63VBMLDKD3JV5HQC5KPVXJQEQHP5TPUMZWNMCZZQ6SKF2OL3"
  );
  await user.type(screen.getByPlaceholderText("0.00"), "100");

  const today = new Date();
  today.setDate(today.getDate() + 1);
  const dateStr = today.toISOString().split("T")[0];
  fireEvent.change(screen.getByDisplayValue(""), { target: { value: dateStr } });

  await user.clear(screen.getByDisplayValue(""));
  fireEvent.change(
    screen.getByRole("spinbutton", { name: /duration/i }),
    { target: { value: "365" } }
  );
}

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

describe("CreateForm", () => {
  beforeEach(() => {
    mockCreateSchedule.mockReset();
  });

  it("renders the form when wallet is connected", () => {
    render(<CreateForm />);
    expect(screen.getByRole("heading", { name: /new vesting schedule/i })).toBeInTheDocument();
  });

  describe("empty beneficiary validation", () => {
    it("does not advance to confirm step when beneficiary is empty", async () => {
      const user = userEvent.setup();
      render(<CreateForm />);

      const submitBtn = screen.getByRole("button", { name: /review & create/i });
      await user.click(submitBtn);

      // Confirm step heading should not appear
      expect(screen.queryByText(/confirm vesting schedule/i)).not.toBeInTheDocument();
      expect(mockCreateSchedule).not.toHaveBeenCalled();
    });
  });

  describe("duration validation", () => {
    it("does not advance to confirm step when duration is 0", async () => {
      const user = userEvent.setup();
      render(<CreateForm />);

      // Fill beneficiary but leave duration at invalid value
      await user.type(
        screen.getByPlaceholderText("G..."),
        "GBSOV3F63VBMLDKD3JV5HQC5KPVXJQEQHP5TPUMZWNMCZZQ6SKF2OL3"
      );
      await user.type(screen.getByPlaceholderText("0.00"), "100");

      const durationInput = screen.getByPlaceholderText("365");
      fireEvent.change(durationInput, { target: { value: "0" } });

      const submitBtn = screen.getByRole("button", { name: /review & create/i });
      await user.click(submitBtn);

      expect(screen.queryByText(/confirm vesting schedule/i)).not.toBeInTheDocument();
      expect(mockCreateSchedule).not.toHaveBeenCalled();
    });
  });

  describe("cliff > duration error", () => {
    it("shows contract error when cliff exceeds duration", async () => {
      mockCreateSchedule.mockRejectedValueOnce(
        new Error("Cliff cannot exceed duration")
      );

      const user = userEvent.setup();
      render(<CreateForm />);

      // Fill all required fields
      await user.type(
        screen.getByPlaceholderText("G..."),
        "GBSOV3F63VBMLDKD3JV5HQC5KPVXJQEQHP5TPUMZWNMCZZQ6SKF2OL3"
      );
      await user.type(screen.getByPlaceholderText("0.00"), "100");

      const dateInputs = document.querySelectorAll('input[type="date"]');
      const today = new Date();
      today.setDate(today.getDate() + 1);
      fireEvent.change(dateInputs[0], {
        target: { value: today.toISOString().split("T")[0] },
      });

      fireEvent.change(screen.getByPlaceholderText("365"), {
        target: { value: "30" },
      });

      // Switch to Cliff mode and set cliff > duration
      await user.click(screen.getByLabelText(/cliff/i));
      fireEvent.change(screen.getByPlaceholderText("180"), {
        target: { value: "90" },
      });

      // Submit to reach confirm step
      await user.click(screen.getByRole("button", { name: /review & create/i }));

      // Now on confirm step — click confirm
      const confirmBtn = await screen.findByRole("button", { name: /confirm & sign/i });
      await user.click(confirmBtn);

      await waitFor(() => {
        expect(
          screen.getByText(/cliff cannot exceed duration/i)
        ).toBeInTheDocument();
      });
    });
  });

  describe("confirm step summary", () => {
    it("renders schedule summary with the entered values", async () => {
      const user = userEvent.setup();
      render(<CreateForm />);

      const beneficiary = "GBSOV3F63VBMLDKD3JV5HQC5KPVXJQEQHP5TPUMZWNMCZZQ6SKF2OL3";
      await user.type(screen.getByPlaceholderText("G..."), beneficiary);
      await user.type(screen.getByPlaceholderText("0.00"), "500");

      const today = new Date();
      today.setDate(today.getDate() + 1);
      const dateStr = today.toISOString().split("T")[0];
      const dateInputs = document.querySelectorAll('input[type="date"]');
      fireEvent.change(dateInputs[0], { target: { value: dateStr } });

      fireEvent.change(screen.getByPlaceholderText("365"), {
        target: { value: "365" },
      });

      await user.click(screen.getByRole("button", { name: /review & create/i }));

      // Confirm step should now be visible
      expect(
        await screen.findByText(/confirm vesting schedule/i)
      ).toBeInTheDocument();

      // Summary items should show the entered values
      expect(screen.getByText(beneficiary)).toBeInTheDocument();
      expect(screen.getByText("365 days")).toBeInTheDocument();
      expect(screen.getByText(/500/)).toBeInTheDocument();
    });
  });
});
