...
export default function CreateForm() {
  ...
  if (!publicKey)
    return (
      <div className="card p-6 sm:p-8 flex flex-col items-center gap-3 text-center">
        ...
        {process.env.NODE_ENV === "development" && (
          <button
            onClick={() =>
              (window as any).setPublicKey?.("GDEBUG" + "0".repeat(50))
            }
            className="mt-2 text-xs text-violet-400 hover:underline"
          >
            Debug: Use Mock Wallet
          </button>
        )}
      </div>
    );
  ...
  if (status === "done")
    return (
      <div className="card p-6 sm:p-8 text-center flex flex-col gap-3">
        <div className="text-4xl">✓</div>
        <p className="text-green-400 font-semibold">Schedule Created!</p>
        <p className="text-zinc-400 text-sm">
          Tokens are now locked and vesting has started.
        </p>
        <a
  ...
  if (step === "confirm") {
    return (
      <div className="card p-4 sm:p-6 flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          <h2 className="text-lg font-semibold">Confirm Vesting Schedule</h2>
          <p className="text-sm text-zinc-400">
            Review the details below before signing the transaction.
          </p>
        </div>

        <div className="flex flex-col gap-4 bg-zinc-900/50 rounded-xl p-3 sm:p-4 border border-zinc-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <SummaryItem
              label="Cliff"
              value={form.kind === "Cliff" ? `${form.cliffDays} days` : "None"}
            />
            <SummaryItem
              label="Revocable"
    ...
    >
      ...
      <Field label="Vesting Type">
        ...
        <p className="text-xs text-zinc-500 mt-1">
          {form.kind === "Linear"
            ? "Tokens unlock gradually over the full duration."
            : "All tokens unlock at once after the cliff period."}
        </p>
      </Field>
      ...
      <label className="flex items-start gap-2 cursor-pointer">
        ...
        <span className="text-sm text-zinc-300 leading-tight">
          Revocable <br className="sm:hidden" />
          <span className="text-zinc-500">
            (you can cancel and recover unvested tokens)
          </span>
        </span>
      </label>
...
function SummaryItem({
  ...
  return (
    <div className={`flex flex-col gap-1 ${full ? "col-span-2" : ""}`}>
      ...
      <span
        className={`text-sm ${full ? "font-mono break-all" : "font-medium break-words"} text-zinc-200`}
      >
        {value}
      </span>
    </div>
